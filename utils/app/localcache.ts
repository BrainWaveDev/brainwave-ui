...
type ResourceType<T> = T extends 'conversation' ? ConversationType :
    T extends 'folder' ? FolderType :
    never;
...
export const get = <T extends ResourceTypeName>(
    ...
    if (data) {
        return { exist: true, resource: JSON.parse(data) as ResourceType<T>['resource']};
    }
